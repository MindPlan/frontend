...
class AuthService {
  ...
  public async sendGoogleJWT(
    token: GoogleTokenType
  ): Promise<AuthGoogleResponse> {
    const data = { credential: token };

    return this.httpService.post<AuthGoogleResponse, { credential: string }>(
      `auth/google/`,
      data
    );
  }
  ...
  public async registration(data: RegistrationRequest): Promise<void> {
    return this.httpService.post<void, RegistrationRequest>(
      'auth/registration/',
      data
    );
  }
